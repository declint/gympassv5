...
//    p.runShellCommandAsynchronously(tempstr);
  ...
  case MODULE_PASSAGE_QUERY_SENT:
    ...
    if (timer_expired(tmr_module_status_change, 5000))
    {
      door1_openlock();
      door2_openlock();
      cvLED(CV5600_LED_GREEN);
      Console.println("query recv, no reply. Failsafe opening");
...
void parse_passage_query_recv()
{
  ...
  if (tempstr_i < 2)
  {
    ...
    door2_openlock();
    cvLED(CV5600_LED_GREEN);
    Console.println("query recv, to short. Failsafe opening");
    return;
  }
  
  //failsafe hittar inte dpv5
  if (strstr(tempstr, "<dpv5>") == 0)
  {
    door1_openlock();
    door2_openlock();
    cvLED(CV5600_LED_GREEN);
    Console.println("query recv, can't find <dpv5>. Failsafe opening");
    return;
  }

  //Find position of <dpv5>
